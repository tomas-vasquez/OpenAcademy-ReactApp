...
class Controller_PayReports extends Controller_admin {
  ...
  */
  ...
  handle_pic_report_selected(img, _callBack) {
    //vemos si la imagen seleccionada es v{alida}

    if (typeof img !== "undefined") {
      if (img.size <= 41943040) {
        // imagenes max 5mb
    ...
    this.payReports.uploadReport(
      ...
      (error) =>
        this.errorsHandler(error, () =>
          this.uploadReport(imgs, form, _myCallBack)
        )
    );
  };
  ...
  */
  ...
  unsafeDeleteMyPayReport = (report_id) => {
    ...
    this.payReports.deletePayReport(
      report_id,
      () => {
      ...
      (error) =>
        this.errorsHandler(error, () => this.unsafeDeleteMyPayReport(report_id))
    );
  };
  ...
  aprovePayReport = (payReport, _callBack) => {
    swal
      .fire({
        ...
        footer:
          "<small className='text-muted'>importante: la imagen seleccionada no deve contener contenido inapropiado</small>",
      })
      .then((result) => {
        if (result.value) {
  ...
  unsafeAprovePayReport = (payReport, _callBack) => {
    ...
    this.payReports.aprovePayReport(
      payReport.report_id,
      () => {
